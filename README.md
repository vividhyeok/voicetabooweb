...
- iOS Safari/Chrome에서는 서버 전사 엔드포인트(`/api/transcribe`) 사용을 위해 **OPENAI_API_KEY** 환경 변수가 반드시 필요
...
- **iOS 호환 모드**: Web Speech API가 없는 기기는 버튼을 눌러 녹음/정지 후 서버 전사(`/api/transcribe`)로 처리 (버튼 비활성화 시 업로드 중 표시)
